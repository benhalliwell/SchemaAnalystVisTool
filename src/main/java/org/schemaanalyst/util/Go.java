...
import org.schemaanalyst.visualise.*;
...
public class Go {
    ...
    public static void main(String[] args){
        ...
        if (gc.visualise) {
	    		...
	    		GUI view = new GUI(report);
	    		view.setupPage();
	        Visualise vis = new Visualise(schemaObject, testSuite, view);
	        List<VisTable> structureTables = vis.getStructureTables();
	        //set default choices 
	        vis.defaultTableChoice(structureTables);
	        vis.defaultColumnChoice(structureTables);
	        JPanel testCasePanel = vis.testCaseTable();
	        JPanel tableList = vis.tableListPanel();
	        JPanel structurePanel = vis.createColumnTable(vis.getChosenStructureTable());
	        JPanel colInfoPanel = vis.columnInformationTab(vis.getChosenStructureTable(), vis.getChosenColumn());
	        JPanel buttonPanel = vis.buttonPanel();
	        SQLWriter sqlWrite = new SQLWriter();
			JTextArea sqlStatement = new JTextArea(sqlWrite.writeCreateTableStatement(schemaObject, schemaObject.getTable(vis.getChosenStructureTable().getTableName())));
			...
			tableSQLPanel.add(SQLScroll);
			//draw the structure and test case pages
	        view.drawStructurePane(tableList, structurePanel, colInfoPanel, tableSQLPanel);
	        view.drawTestCasePane(testCasePanel, buttonPanel);
        	}
